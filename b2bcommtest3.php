...
class icmModel {
    ...
    public function __construct() {             
        ...
        try{
            $this->posted = $_POST;  // keep initial array separate from post array
			...
			if($this->posted[self::pv_market]=='GL'){  // as no GL will be found in the country table, create it here
				$this->market 	= (object) array ( "LookupName"=>"GL", "Name"=>"Global", "ISOCode"=>"GL" );
			} else {
        ...
        try{    
            ...
            // ---------------------------------------------------------------------------------------------
                ...
                foreach($iAccount as $singleAccount) {
                    ...
                    $accountOrg = array(           
                        "ICSPOrg"=> nullq($this->get_ICM_section($iAddress,"name","ICSPOrg","tagData")),
                        "ICSPOrgType"=> nullq($this->get_ICM_section($iAddress,"name","ICSPOrgType","tagData")),
                        "ICSPOrgID"=> nullq($this->get_ICM_section($iAddress,"name","ICSPOrgId","tagData"))
                        );
                    // "Contact_ID"=>"12345",
                ...
                if(strlen($val)>0){
                    switch($key){
                        ...
                        case "ICSPWorkPhone" : $contact->CustomFields->CO->ICSPWorkPhone = $data["ICSPWorkPhone"];break;
 						Case "Market" : $contact->CustomFields->Market->market_name->LookupName = $this->market->Name;break;                 
                        case "Email" : 
                            //Add email address
